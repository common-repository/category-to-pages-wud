...
if(!function_exists('cattopage_wud_widget_urls')){
	...
	if ( ! empty( $args ) && ! is_wp_error( $args ) ) {
		...
		$showiconcat = "";
		$showicontag = "" ;
		if(get_option('cattopage_wud_icon') == 0 ){
			$showiconcat = "<span class='wudicon wudicon-category' >  </span>";
		}
		if(get_option('cattopage_wud_icon') == 0 ){
			$showicontag = "<span class='wudicon wudicon-tag' >  </span>";
		}			
			if(!empty($cat_tag) && ($cat_tag=="categories" || $cat_tag=="category")){
				$term_list .= '<'.$cattopage_wud_widget_font1.'><br>'.$showiconcat.$cattopage_wud_widget_title1.'</'.$cattopage_wud_widget_font1.'>';
			...
			if(!empty($cat_tag) && ($cat_tag=="tags" || $cat_tag=="post_tag")){
				$term_list .= '<'.$cattopage_wud_widget_font2.'><br>'.$showicontag.$cattopage_wud_widget_title2.'</'.$cattopage_wud_widget_font2.'>';
		...
		//-> CAT OR TAG
				...
				if($cattopage_wud_widget_option2=="1"){
					$term_list .= '&nbsp;&nbsp;<a ClickResult="'.$cattopage_wud_cnt.'" class="cattopage_wud_split" style="float: left; text-decoration: none; box-shadow: none;" id="cattopage_wud_split"><span><img src="'.plugin_dir_url( dirname( __FILE__ ) ) . "images/wud_button.png".'"></span></a> ';
				...
				$term_list .= '<a href="' . esc_url( get_term_link( $pterm ) ) . '" style="text-decoration: none; box-shadow: none;">' . $pterm->name . '</a><br>';
					if(!empty($return)){$term_list .='</b>';}
				...
				if($cattopage_wud_widget_option1=="1"){	
					...
					if($term_parent==0){$term_list .= '&nbsp;&nbsp;&nbsp;&nbsp;&#8627;&nbsp;<a href="'.esc_url(get_permalink($postwud->ID)).'" style="text-decoration: none; box-shadow: none;">'.$postwud->post_title.'</a><br>';}			
					}
					if($cattopage_wud_widget_option2=="1"){
						$term_list .= '</div>';
					}
		...
		//-> SUB CAT OR SUB TAG
					...
					if($cattopage_wud_widget_option2=="1"){
						$term_list .= '<a ClickResult="'.$cattopage_wud_cnt.'" class="cattopage_wud_split" style="text-decoration: none; box-shadow: none;" id="cattopage_wud_split"><span><img src="'.plugin_dir_url( dirname( __FILE__ ) ) . "images/wud_button.png".'"></span></a> ';
					}
					
					//If current page haves this category or tag
					$return = is_object_in_term( $post->ID, $cat_tag, $term->slug );
						if(!empty($return)){$term_list .='<b>';} 
					$term_list .= '&#9492; &nbsp;<a href="' . esc_url( get_term_link( $term ) ) . '" style="text-decoration: none; box-shadow: none;">' . $term->name . '</a><br>'; 
						if(!empty($return)){$term_list .='</b>';}
					...
					if($cattopage_wud_widget_option1=="1"){	
						...
						$myposts = get_posts( $argspost );
						foreach ( $myposts as $postwud ){ 
						$term_list .= '&nbsp;&nbsp;&nbsp;&nbsp;&#8627;&nbsp;<a href="'.esc_url(get_permalink($postwud->ID)).'" style="text-decoration: none; box-shadow: none;">'.$postwud->post_title.'</a><br>';
						}
						if($cattopage_wud_widget_option2=="1"){
							$term_list .= '</div>';
						}
